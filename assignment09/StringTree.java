...
public class StringTree {
	...
	public List<String> listContent() {
		...
		list.add(data);
		for(StringTree st : children) {
			list.addAll(st.listContent());
		}
		return list;
	}
	...
	public StringTree treeDelete(String str) {
		...
		// return this
		if(!data.equals(str) && children.size() == 0) {
			return this;
		} else if (data.equals(str)) {
			if(children.size() == 0) {
				return null;
			}
			StringTree newRoot = children.get(0);
