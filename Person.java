...
public class Person {
	...
	public void printHistory() {
		...
		if (history[1] != null) {
			...
			UniversityStudent uss = (UniversityStudent) (history[1]);
			// Use the similar approach to "cast" history[1] to a UniversityStudent object.
			// From that object, the getUniversity method gives you the university and from
			// that
			// the getter methods will give the name and city of the university.

			System.out.println("\tUniversity: Duke" + " in " + uss.getUniversity().getcity());
			// In the System.out.println, start with "\tUniversity: " + followed by the name
			// of the university
		...
		if (history[2] != null) {
			...
			System.out.format("\tJob at %s in %s, with a salary of %.2f\n", e.getCompany().getName(),
					e.getCompany().getCity(), e.getSalary());
		}
	}
