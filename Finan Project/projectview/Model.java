...
public class Model {
	...
	private final Instruction[] INSTR = new Instruction[0x16];
 	...
 	public Model() {//no-argument constructor
 		this(() -> System.exit(0));
 	}
 	public Model(HaltCallBack cb) {//value constructor
 		callBack = cb;
 		...
 		INSTR[0x9] = (arg, mode) -> {
 			...
 			if (mode != DIRECT) {
 				...
 				if (arg < 0) {cpu.accumulator = 1;} else {
 					cpu.accumulator = 0;
 				}
 	...
 	public void changeToJob(int i) { 
		if (i !=0 || i!=1 || i!= 2 || i != 3) {
			throw new IllegalArgumentException("Illegal Job Number");
		}
	...
	public static final Map<String, Integer> OPCODES = Map.ofEntries(
				entry("NOP", 0), entry("LOD", 1), entry("STO", 2), entry("ADD", 3), entry("SUB", 4), entry("MUL",5),
				entry("DIV",6), entry("AND",7), entry("NOT",8), entry("CMPL",9),entry("CMPZ", 10), entry("JUMP", 11),
